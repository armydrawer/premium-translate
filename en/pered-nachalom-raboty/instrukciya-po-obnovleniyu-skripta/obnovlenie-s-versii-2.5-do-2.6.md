...
{% hint style="warning" %}
Before updating the script, please update the Ioncube Loader on your server to version 13.0 or higher. If your current version is below 13, see the [**instructions to check your installed version**](https://premium.gitbook.io/main/osnovnye-nastroiki/faq/kak-proverit-versiyu-ioncube-ustanovlennuyu-na-servere) and the [**instructions to update it**](https://premium.gitbook.io/main/osnovnye-nastroiki/faq/kak-obnovit-ioncube-loader). Your hosting provider’s technical support can assist you with this update.
{% endhint %}

{% hint style="warning" %}
Starting with version 2.6, support for **PHP 7.1, 7.2, 7.3, and 7.4** has been removed for security reasons, and support for **PHP 8.2** has been added. If you were using PHP 7.4 or earlier with version 2.5, you must update your server’s PHP version before updating the script. We recommend contacting your hosting provider’s support team to perform the PHP upgrade.
...
{% hint style="warning" %}
If you are using merchant and auto-payout modules developed specifically for you by us, please request the updated modules in your Telegram group (**do not request them through the support bot**).

If you are using merchant, auto-payout, or other modules from third-party developers, these will not work with version 2.6 unless updated by their developers.
{% endhint %}

{% hint style="warning" %}
If you used the Electrum and/or "Trading Actions" modules in version 2.5, please request the updated modules for version 2.6 in your Telegram group (**do not request them through the support bot**).
{% endhint %}
...
To do this, navigate on your server to the folder `www/<site_name>/wp-content/themes/`, find your theme’s folder, archive it, download it to your computer, and then send the archive to your Telegram group.  
{% endhint %}

1. In the exchanger control panel, go to the "**Console**" section and enable technical mode to prevent users from making exchange requests on the site during the script update.
...
* **`/flags/`**  
* **`/languages/`**  
* **`/moduls/`** (please read the note below before proceeding)
...
5. Go to the "[**Your Scripts**](https://premiumexchanger.com/uscripts/)" section and download the archive with the **update files for version 2.6** matching your PHP version.
...
You can set a different number if needed, but if the cycle with the specified number of requests is too resource-intensive for your server, it may cause an error.
{% endhint %}

{% hint style="info" %}
You may notice "**Technical Step X**" buttons next to the regular "**Step X**" buttons. Before executing each step, the system calculates the number of requests required. Sometimes this number can be very large, and the server might struggle to perform the calculation. In such cases, instead of using the "**Step X**" button, use the "**Technical Step X**" button. This allows you to manually specify the number of requests without the server calculating it.

If you use a **technical step**, you must manually enter the number of requests. We recommend setting a deliberately high number, such as 100,000.
{% endhint %}
...
{% hint style="warning" %}
Update mode is automatically enabled every time the main plugin is deactivated and then reactivated, so you always need to disable it manually.
{% endhint %}

11. If you are using the **Parsers 2.0** or **BestChange parser** modules, after disabling update mode you must manually start the parsers by clicking the Cron link in their respective sections.  
    To run Parsers 2.0:
...
![image1](<../../.gitbook/assets/image (1644).png>) ![image2](<../../.gitbook/assets/image (1645).png>)

It is **mandatory** to switch to the new **"Internal Account Module" (iac)**.
...
* Perform the migration steps sequentially — steps 7 and 8 under "**Exchange Settings**" -> "**Migration**".

![image3](<../../.gitbook/assets/image (1646).png>)

![image4](<../../.gitbook/assets/image (1647).png>) ![image5](<../../.gitbook/assets/image (1648).png>)
...
![image6](<../../.gitbook/assets/image (1649).png>)

* Remove the **domacc** merchant and auto payout modules from the admin panel under "**Merchants** -> **Merchants**" and "**Merchants** -> **Auto Payouts**", then delete these modules from the server (module folder paths: `wp-content/plugins/premiumbox/moduls/merchants/domacc` and `wp-content/plugins/premiumbox/moduls/paymerchants/domacc`).
* Add the new **iac** merchant and auto payout modules in the "**Merchants** -> **Merchants**" and "**Merchants** -> **Auto Payouts**" sections, and configure them according to the [instructions](https://premium.gitbook.io/main/osnovnye-nastroiki/nastroiki/vnutrennie-scheta).
* Add the newly created merchants to the appropriate exchange directions (under the "**Merchants and Payouts**" tab in the exchange direction settings).

![image7](<../../.gitbook/assets/image (463).png>)

---

If you need any further assistance with the migration, please refer to the detailed guide or contact support.
...
3. Starting from version 2.6, the **"Captcha for site (image selection)" (sitecaptcha_img)** module automatically generates the options for users to choose from on the website. In the previous version, you could create your own tasks, but since version 2.6 this option has been disabled, so any changes to the captcha’s appearance will immediately be reflected on the site.  
