...
* **Blacklist:** Updated similarly to the **blacklist_bestchange** module, allowing the system to accept funds but stop payouts if the user is on the blacklist. Module settings are located under "**Blacklist**" -> "**Settings**."
...
- **AML Check:** Added the option to perform a risk check immediately before sending funds to the client’s wallet, with the order being marked as an error if the risk level is exceeded. Risk level settings are available under "**Modules**" -> "**AML Bot**" or "**Getblock**" (depending on which service you have integrated).

  The logic for performing the check can be configured in the exchange direction settings, under the "**AML Bot**" or "**Getblock**" tab.
...
* Added the ability to insert an example image for card verification in the currency settings using the img shortcode.
...
* A list of application statuses that the merchant will recognize when receiving payment notifications from the payment system. If none of the options are selected, the merchant will operate with the statuses configured in version 2.4.
