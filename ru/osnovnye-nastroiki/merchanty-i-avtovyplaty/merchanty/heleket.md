...
• **Invoice** — временный адрес кошелька, создаваемый для конкретной сделки, имеет ограничение по времени жизни после выдачи в заявке (прием средств на этот кошелек **ограничен по времени** — 12 часов, начиная с момента выдачи в заявке). _Для этого типа кошелька требуется создать задание cron на сервере._\
• **Address** — постоянный адрес кошелька, который привязывается к вашему ЛК у мерчанта, без ограничения по времени жизни (прием средств на этот кошелек **не ограничен по времени).** _Статус оплаты проверяется по коллбэку, задание Cron для этого типа кошелька не требуется._
...
![](<../../../.gitbook/assets/image (2085).png>)<br>

Список валют, для которых можно подключить автоконвертацию:

<img src="../../../.gitbook/assets/image (2086).png" alt="" data-size="original">
{% endhint %}
