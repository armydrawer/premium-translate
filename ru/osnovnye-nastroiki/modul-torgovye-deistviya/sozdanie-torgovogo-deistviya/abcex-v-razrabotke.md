...
{% hint style="danger" %}
Обратите внимание, что ABCEx рассчитывает курс для пары USDT-BTC с учетом маркапа (наценки) в 5% (а также 0.25% для пары USDT-RUB) — за счет этого сервис может гаранировать исполнение ордеров даже при резком изменений курса в момента.

Сделка будет происходить по курсу в стакане, вы можете рассчитывать объем ордера с учетом вышеуказанных % резерва, чтобы откупалось нужное вам количество валюты.
{% endhint %}
