...
{% hint style="info" %}
Например, вы посчитали, что прибыль в этом направлении = 1%. Вы указываете это значение в поле "**Прибыль" -> "из суммы "Получаю"**. Система при совершении обмена вычисляет от итоговой суммы к получению 1%, переводит в доллары — это и является прибылью обменного пункта, которой обменник будет делиться с партнерами.
{% endhint %}
