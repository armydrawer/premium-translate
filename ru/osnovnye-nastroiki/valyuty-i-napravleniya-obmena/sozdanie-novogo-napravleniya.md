...
* **Минимальная/максимальная сумма** — укажите суммы, которыми будет ограничен обмен для данного города (опция распространяется только на XML-файл с направлениями обмена)
...
**Макс. сумма комиссии** — укажите максимальную сумму комиссии, которую может списать платежная система за перевод средств, если такая сумма предусмотрена в платежной системе. Если указать «**0**» (ноль), то ограничений не будет. Если ограничение установлено, то некоторые поля в форме создания заявки станут неактивными.
...
**Шаблон** _—_ шаблон для текста для его передачи через шорткод `[dirtemp]` в письмах по заявкам для этого направления обмена
...
**Мерчант** — если вы хотите, чтобы оплата по заявке происходила через мерчант, то выберите  соответствующий мерчант для приема оплаты от клиента. Мерчант позволяет принимать оплату через сайт платежной системы. Если мерчант не выбран, то клиент должен будет произвести оплату вручную по инструкции, которую он увидит после создании заявки. Читайте подробнее про [настройки мерчантов](https://premium.gitbook.io/rukovodstvo-polzovatelya/navigaciya/merchanty-i-vyplaty/merchanty).

**Суточный лимит для мерчанта** — установите суточный лимит на прием средств для мерчанта. Мерчант не сможете принять оплату больше заданного лимита.

**Месячный лимит для мерчанта** — установите месячный лимит на прием средств для мерчанта. Мерчант не сможете принять оплату больше заданного лимита.

**Макс. сумма платежа для одной заявки** — установите максимальную сумму платежа для одной заявки. Мерчант не сможете принять оплату больше заданного лимита.

**Автовыплаты** — если вы хотите, чтобы система выплачивала деньги клиенту по заявке автоматически, то выберите название соответствующей платежной системы через которую будет осуществляться авто выплата. Если вы ничего не выберите, то выплаты по заявкам необходимо производить вручную администратору/оператору обменного пункта. Читайте подробнее про [настройки автовыплат](https://premium.gitbook.io/rukovodstvo-polzovatelya/navigaciya/merchanty-i-vyplaty/vyplaty).
...
**Мин. сумма выплаты партнеру** — минимальная сумма, которую партнер может запросить для вывода.

**Макс. сумма выплаты партнеру** — максимальная сумма, которую партнер может запросить для вывода.
